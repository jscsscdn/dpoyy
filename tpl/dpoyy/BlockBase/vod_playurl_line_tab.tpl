...
<div class="detail-source">
	...
	<div class="detail-content tab-content ff-playurl-tab" id="detail-content">
		...
		<ul class="detail-play-list clearfix tab-pane ff-playurl ff-playurl-tab-{$feifei.player_sid} {$active}" data-active="{$vod_id}-{$play_sid}-{$play_pid}" data-more="{$Think.config.ui_playurl|intval}">
			...
			<li data-id="{$vod_id}-{$feifei.player_sid}-{$pid}"><a href="{:ff_url_play($list_id,$list_dir,$vod_id,$vod_ename,$feifei['player_sid'],$pid)}" title="{$feifeison.title}"><if condition="(strlen($feifeison['title']) eq 8) and is_numeric($feifeison['title'])">{$feifeison.title|msubstr=2,5}<else/>{$feifeison.title|msubstr=0,6}</if></a></li>
			</volist>
		</ul>
		</volist>
	</div>
