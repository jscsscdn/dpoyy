...
<div class="detail clearfix">
	...
	<div class="detail-poster">
		<a href="{:ff_url_read_vod($list_id,$list_dir,$vod_id,$vod_ename,$vod_jumpurl)}"><img src="{$vod_pic|ff_url_img=$vod_content}" alt="{$vod_name}海报剧照"></a>
	</div>
	...
	<ul class="detail-actor clearfix">
		...
		<notempty name="vod_writer"><p><label>编剧：</label>{$vod_writer|ff_url_search='writer'}</p></notempty>
		...
		<li class="clearfix">
		<p><label><span class="hidden-xs">神马</span>评分：</label><span class="text-dark">{$vod_gold}</span></p>
		<p><label><span class="hidden-xs">发行</span>年份：</label><include file="BlockBase:vod_year" /></p>
		<p class="hidden-xs"><label>更新时间：</label><span class="text-dark">{$vod_addtime|date='Y-m-d',###}</span></p>
		</li>
