...
if($search_wd){
  ...
  $page = ff_url_page('vod/search', $params, true, 'search', 4);
	...
	$item_vod = ff_mysql_vod('writer:'.$search_writer.';limit:42;page_is:true;page_id:search;page_p:'.$search_page.';cache_name:default;cache_time:default;order:vod_'.$search_order.';sort:desc');
  $params = array('writer'=>urlencode($search_writer),'p'=>'FFLINK');
  $page = ff_url_page('vod/search', $params, true, 'search', 4);
}else if($search_name){
